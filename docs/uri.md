...
## Формат OlConnect Manager

Admin UI панели `OlConnect Manager` генерирует URI в собственном формате для клиента `OlConnect` (а также `openflux://` для L3-туннелей). Формат содержит необходимые метаданные и параметры подключения.

### Формат

```text
olconnect://<Auth>@r/<RoomID>?k=<EncryptionKey>&t=<Transport>&f=<FPS>&b=<Batch>&core=legacy&c=<ClientID>#<Name>
```
*(Также поддерживается обратная совместимость со схемой `olcrtc://`)*

API-эндпоинты:
- `GET /api/instances/{id}/uri` — основной формат (`olconnect://...` или `openflux://...`)
- `GET /api/instances/{id}/qr` — QR-код для быстрого импорта в клиент
